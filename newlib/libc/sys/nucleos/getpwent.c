...
/* Read one entry from the password file. */
{
	...
	for (;;) {
		...
		if ((p= scan_colon()) == 0) continue;
		entry.pw_uid= strtol(p, 0, 0);
		if ((p= scan_colon()) == 0) continue;
		entry.pw_gid= strtol(p, 0, 0);
...
/* @nucleos: must be enough for now */
weak_alias(getpwent, getpwent_r);
