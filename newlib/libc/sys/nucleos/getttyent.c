...
/* Read a field that may be a quoted list of words. */
{
	...
	if (*lineptr == '"') {
		...
		lineptr++;
		while ((p= scan_white(1)) != 0 && *p != '"') {
			if (argvptr == arraylimit(ttargv)) return 0;
			*argvptr++= p;
		}
		if (*lineptr == '"') *lineptr++= 0;
	} else {
		/* Just one word. */
		if ((p= scan_white(0)) == 0) return 0;
		if (argvptr == arraylimit(ttargv)) return 0;
		*argvptr++= p;
	}
	if (argvptr == arraylimit(ttargv)) return 0;
...
/* Read one entry from the ttytab file. */
{
	...
	if (ttfd < 0 && setttyent() < 0) return 0;

	/* Look for a line with something on it. */
	for (;;) {
		if (!getline()) return 0;	/* EOF or corrupt. */

		if ((entry.ty_name= scan_white(0)) == 0) continue;
...
/* Return the ttytab file entry for a given tty. */
{
	...
	endttyent();
	while ((tty= getttyent()) != 0 && strcmp(tty->ty_name, name) != 0) {}
	endttyent();
	return tty;
}
